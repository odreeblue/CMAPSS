...
class Regressor(nn.Module):
    ...
    def forward(self, x): # 모델 연산의 순서를 정의
        ...
        return x
sensor = dict()
sensor[1]=[2,3,4,9,11,15,17] #1

sensor[2]=[2,3,4,9,11,15,17] #5

#sensor[3]=[2,3,4,6,7,8,9,11,12,13,15,16,17,20,21]
sensor[3]=[2,3,4,9,11,15,17] #2

#sensor[4]=[2,3,4,7,8,9,11,12,13,15,17,20,21]
sensor[4]=[2,3,4,6,7,8,9,11,12,13,15,16,17,20,21] #3

#sensor[5]=[2,3,4,9,11,15,17]
sensor[5]=[2,3,4,7,8,9,11,12,13,15,17,20,21] #4

sensor[6]=[2,3,4,6,7,8,9,11,12,13,15,17,20,21] #6
...
if sys.argv[1] =='train':
    ...
    print("잘못된 명령행 인자임. train OR test 입력 바람")

TestData[1] = df[(df['type']==type)&(df['regime']==0)]
TestData[2] = df[(df['type']==type)&(df['regime']==1)]
TestData[3] = df[(df['type']==type)&(df['regime']==2)]
TestData[4] = df[(df['type']==type)&(df['regime']==3)]
TestData[5] = df[(df['type']==type)&(df['regime']==4)]
TestData[6] = df[(df['type']==type)&(df['regime']==5)]
...
for i in range(1,7):
    ...
    print(str(i)+' 번째')
    print(type_name+'_rmse: '+str(test_rmse[i])+', '+type_name+'_pred의 shape: '+str(test_predValue[i].shape))
...
directory = '13_'+type_name+'data_predicted'
...
for i in range(1,7):
    R_squared = COD(TestData[i]['target'],TestData[i]['predValue'])
    RGraph(i,TestData[i]['predValue'],TestData[i]['target'],R_squared,directory)
    filepath = './'+directory+'/'+type_name+'PredictedValue_regime'+str(i)+'.txt'
    np.savetxt(filepath,TestData[i],delimiter='\t',newline='\n',fmt='%1.6e')

directory = '13_50,100,200,260Engine_RUL_'+type_name
if os.path.isdir(directory):
    print(directory+'폴더 있음')
else:
    print(directory+'폴더 없음, 생성함')
