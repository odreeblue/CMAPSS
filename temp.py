...
RUL_FD002 = np.loadtxt("./11_Testdata/RUL_FD002.txt",dtype='float',delimeter=" ")

# 2) FD002 데이터의 Unit 열과 Timestep 열을 DataFrame으로 변환
columns = list(['unit','timestep']) 
data = pd.DataFrame(test_FD002[:,0:2],columns=columns)

# 3) 최대 Unit 번호 구하기
maxUnitNum = data['unit'].max() # maybe 260개
print("최대 유닛 번호 : ",maxUnitNum)


# 4) Unit별 최대 사이클수 구하기
maxCycle = dict()
for i in range(1,int(maxUnitNum)+1):
    maxCycle[i] = RUL_FD002[i]
