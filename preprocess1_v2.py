...
x_columns2 = ['set1','set2','set3']
kMeansModels2 =dict() #k값 별 모델 저장할 딕셔너리
kMeansModelPreds2 = dict() # k값별 모델 예측 결과 저장할 딕셔너리
#kMeansModelLabelEncoder = dict() # k 값별 라벨인코더 저장할 딕셔너리
sumSquaredDistancesList2 = list() # 샘플과 클러스터 센터간 거리 제곱의 합 리스트
silhouetteScoreList2 = list() # Silhouette Coefficient 평균 리스트
...
for k in ks:
    ...
    model = KMeans(n_clusters=k, random_state=0, n_init=100) # n_init: 초기 중심 위치 시도 횟수
                                                             # random_state : 시드값
    cluster_labels = model.fit_predict(FD002_df[x_columns2]) # X 컬럼으로 지정된 필드갑으로 피팅
    
    kMeansModels2[k] = model
    kMeansModelPreds2[k] = cluster_labels

    sumSquaredDistancesList2.append(model.inertia_)    # # 샘플과 클러스터 센터간 거리 제곱의 합 저장
    silhouetteScoreList2.append(silhouette_score(FD002_df[x_columns].values, cluster_labels)) # Silhouette Score저장
...
#print(np.shape(kMeansModelPreds[6]))
...
temp = np.concatenate((kMeansModelPreds[6].reshape(-1,1),kMeansModelPreds2[6].reshape(-1,1)),axis=1)
np.savetxt('./01_FD002_Reg_data/Compare_Reg.txt',temp,delimiter='\t',newline='\n',fmt='%1.2e')
